...
`send.py` checks bridge health, starts it automatically if needed (hidden console window),
and launches a taskbar tray monitor. No manual server management required.
...
To reset a session: delete the corresponding `~/.claude/ai-bridge/cookies/<site>.json` — the bridge will re-prompt on next request.
...
- **Skip redundant navigation** — detects if chat input is already present after cookie-based launch
...
| `GET /inspect/{site}?write=true` | Save heuristic suggestions to site YAML |
| `GET /debug/selectors/{site}` | Diagnose CSS selector issues |
...
| `POST /session/notify-login/{site}` | Signal manual login complete |
| `GET /session/status/{site}` | Check session/login-pending state |
...
Login flow:
1. `uv run python proxy/send.py perplexity "test"` — bridge starts, browser opens at site
...
Prefer `proxy/send.py` over `curl` — handles bridge lifecycle and Windows encoding:

```bash
# Basic usage — bridge auto-starts if needed
...
# OpenRouter (API-based, no browser)
...
When `send.py` auto-starts the bridge, it also launches a lightweight tkinter tray GUI (minimized to taskbar):

- Green/red status indicator (polls `/health` every 5s)
- Uptime counter
- "Stop ai-bridge" button (terminates bridge process)
